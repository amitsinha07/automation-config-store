...
export async function createMockResponseFIS12_200(
	...
	// Remove FIS12/ prefix if present since we're already in the FIS12 directory
	const defaultPath = api_details.default.replace(/^FIS12\//, '');
	const default_message = yamlToJson(
		path.resolve(__dirname, `../../${defaultPath}`)
	);
	const payload: any = {
		context: context,
