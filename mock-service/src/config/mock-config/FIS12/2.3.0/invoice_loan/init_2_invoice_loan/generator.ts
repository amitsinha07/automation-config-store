...
/**
 * Init 2 Generator for Invoice Loan
 * 
 * Logic:
...
export async function initInvoiceLoanGenerator(existingPayload: any, sessionData: SessionData) {
    console.log("init_2_invoice_loan generator - Session data:", sessionData);
    ...
    const submission_id = (sessionData as any)?.form_data?.entity_kyc_form_verification_status?.form_submission_id || (sessionData as any).entity_kyc_form_verification_status;
    if (existingPayload.message?.order?.items?.[0]?.xinput?.form) {
        existingPayload.message.order.items[0].xinput.form.id = "entity_kyc_form_verification_status";
        console.log("Updated form ID to entity_kyc_form_verification_status");
    }
    ...
    console.log("init_2_invoice_loan payload generated successfully");
