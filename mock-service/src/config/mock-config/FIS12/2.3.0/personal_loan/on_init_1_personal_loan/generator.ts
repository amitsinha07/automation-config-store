...
/**
 * On Init 1 Generator for Personal Loan
 * 
 * Logic:
...
export async function onInitPersonalLoanGenerator(existingPayload: any, sessionData: SessionData) {
    console.log("on_init_1_personal_loan generator - Session data:", sessionData);
    ...
    if (existingPayload.message?.order?.items?.[0]?.xinput?.form) {
        const url = `${process.env.FORM_SERVICE}/forms/${(sessionData as any).domain}/kyc_form_verification_status?session_id=${(sessionData as any).session_id}&flow_id=${(sessionData as any).flow_id}&transaction_id=${existingPayload.context.transaction_id}`;
    ...
    console.log("on_init_1_personal_loan payload generated successfully");
