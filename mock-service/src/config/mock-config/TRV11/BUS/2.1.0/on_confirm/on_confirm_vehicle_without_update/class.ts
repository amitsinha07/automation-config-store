...
import { onConfirmVehConfQrGenerator } from "./generatorQr";
import { onConfirmVehConfGenerator } from "./generator";

export class MockOnConfirmVehConWithoutUpdateBus210Class extends MockAction {
  ...
  name(): string {
    return "on_confirm_veh_con_BUS_210";
  }
  get description(): string {
    return "Mock for on_confirm_BUS_210";
  }
  generator(existingPayload: any, sessionData: SessionData): Promise<any> {
    return onConfirmVehConfGenerator(existingPayload, sessionData);
  }
  async validate(
  targetPayload: any,
...
export class MockOnConfirmVehConBusQr210Class extends MockAction {
  get saveData(): saveType {
    return yaml.load(
      readFileSync(path.resolve(__dirname, "../save-data.yaml"), "utf8")
    ) as saveType;
  ...
  generator(existingPayload: any, sessionData: SessionData): Promise<any> {
    return onConfirmVehConfQrGenerator(existingPayload, sessionData);
  }
  async validate(
  targetPayload: any,
  sessionData: SessionData
