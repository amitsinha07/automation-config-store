...
import { onConfirmDelayedGenerator } from "./generator";

export class MockOnConfirmDelayedMetro210Class extends MockAction {
  ...
  name(): string {
    return "on_confirm_delayed_METRO_210";
  }
  get description(): string {
    return "Mock for on_confirm_METRO_210";
  }
  generator(existingPayload: any, sessionData: SessionData): Promise<any> {
    return onConfirmDelayedGenerator(existingPayload, sessionData);
  }
  async validate(
  targetPayload: any,
  ...
  if (!sessionData.payment_id) {
    return {
      valid: false,
      message: "No payment_id available in session data",
