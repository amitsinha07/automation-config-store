...
const createQuoteFromItems = (items: any): any => {
  ...
  breakup.push(taxBreakup);

  // Add OTHER_CHARGES object to breakup
  const otherChargesBreakup = {
    title: "OTHER_CHARGES",
    price: {
      currency: items[0]?.price.currency || "INR",
      value: "0",
  ...
  breakup.push(otherChargesBreakup);

  // Calculate final price by summing all breakup prices
  const finalPrice = breakup.reduce((sum: number, item: any) => {
    return sum + Number(item.price.value);
  }, 0);

  return {
    price: {
      value: finalPrice.toFixed(2), // Total price as a string with two decimal places
...
export async function onSelectGenerator(
  ...
  existingPayload.message.order.provider = {
    ...
    time: sessionData?. provider_time ?? {
      range: {
        start: "2026-01-25T00:30:00.000Z",
        end: "2026-01-25T18:30:00.000Z",
      },
    },
  };
